...
.data                  
    ...
    mem_released db 13, 10, "Memory released successfully$"
  ...
  ; сообщения об ошибках
    ...
    position_in_cmd dw 0                ; Позиция в cmd_text
...
run_loop:
    ...
    mov ah, 4Ah             ; Функция DOS 4Ah - изменить размер блока памяти
...
epb dw 0          ; Блок параметров среды (EPB)
    dw offset 0   ; Командная строка (пустая)
    dw 5Ch, 0     ; FCB1
    dw 6Ch, 0     ; FCB2
...
;;;;;;;     push cx
;;;;;;;     ; Освобождаем память 
;;;;;;;     ; mov bx, ((program_length/16) + 17) + ((dsize/16) + 17) + 1 ; Расчет нового размера
;;;;;;;     ; mov bx, ((program_length + 100h + 200h) + 1 ; Расчет нового размера
;;;;;;;     ; mov     bx,     1000h   
;;;;;;;     ; mov bx, 100h
;;;;;;
;;;;;;;     ; mov bx, offset program_length
;;;;;;;     ; mov cl,4    ; вычисляем длину программы в параграфах
;;;;;;;     ; shr bx,cl
;;;;;;;     ; add bx,17   ; добавляем 1 параграф для выравнивания и 256 байт для стека
;;;;;;
;;;;;;;     ; mov bx, cs  ; Сегмент кода
;;;;;;;     ; mov ax, ds  ; Сегмент данных
;;;;;;;     ; sub ax, bx  ; Размер кода в параграфах
;;;;;;;     ; add ax, 16  ; Добавляем размер данных (выровненный до параграфа)
;;;;;;;     ; mov bx, ax
;;;;;;
;;;;;;;     ; mov ax, 4A00h
;;;;;;;     ; int 21h
;;;;;;;     ; jc memory_error
;;;;;;    
;;;;;;;     ; show_str mem_released
;;;;;;;     ; Запуск программы
;;;;;;;     ; Подготовка к запуску программы
;;;;;;;     ; mov ax, @data  
;;;;;;;     ; mov es, ax              ; Устанавливаем ES на сегмент данных
;;;;;;
;;;;;;;     mov ah, 4Bh             ; Функция DOS 4Bh - запуск программы
;;;;;;;     mov al, 00h             ; Подфункция 00h - загрузка и выполнение
;;;;;;;     lea dx, buffer    ; Имя программы для запуска
;;;;;;;     mov bx, 0             ; Блок параметров среды (EPB)
;;;;;;;     int 21h
;;;;;;;     ; mov dx, offset buffer  ; DS:DX = имя программы
;;;;;;;     ; mov bx, 0
;;;;;;;     ; mov ax, 4B00h         ; Функция EXEC
;;;;;;;     ; int 21h
;;;;;;;     jc exec_failed
...
;     mov cx, num1           ; N - количество запусков
    
; run_loop:
;     push cx
    
;     ; Подготовка параметров для запуска
;     mov ax, 4B00h          ; Функция DOS: запуск программы
;     mov dx, offset buffer  ; Имя программы
;     mov bx, 0              ; Нет параметров командной строки
;     int 21h
;     jc run_error
