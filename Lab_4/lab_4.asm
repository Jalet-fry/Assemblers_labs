...
.data 
    ...
    program_name db "hello.exe", 0                    ; Имя запускаемой программы с нулевым окончанием
    welcome db "Usage:test.exe path.txt N K", '$'
    die db "Finally we will die!", '$'
  ...
  ; сообщения об ошибках
    ...
    num1 dw 2                           ; Первое число (1-255)
    num2 dw 1                           ; Второе число (1-255)
    lines_counter dw 0                  ; Счетчик строк, удовлетворяющих условию

    psp_seg dw ?                                     ; Сегмент psp
    ...
    dsize = $ - program_name                             ; Размер сегмента данных в байтах
.code
...
print_binary proc
    ...
    mov bx, ax      ; Исходное число из ax в bx
...
conv_loop:
    ...
    mul bl        ; Умножаем текущий результат на 10
    ; push dx
    ; IMUL bx      ; Стало (для 8086)
    jc conv_error      ; Если переполнение - ошибка
    ; pop dx

    add al, dl         ; Добавляем новую цифру
    ; show_str second 
    ; show_str we_are_in_conv 
    ; call print_binary
    jc conv_error      ; Если переполнение - ошибка
...
;     ; mov bx,(csize+dsize)/16 + 2  ; Расчет нового размера
...
command_line proc
    ...
    mov si, 81h             ; Адрес длины командной строки в PSP
...
command_line_input:
    ...
    je command_line_end
    mov cmd_text[di], al      ; Сохраняем символ в буфере
...
start:
    ...
    ; mov ax, es
    ; mov psp_seg, ax           ;Сохраняем первоначальный ES в psp_seg
    ; mov ax, @data            
    ; ; call read_cmd
    ; call command_line
    ; call start_another_program
    call start_another_program
    ; mov ax, psp_seg
    ; mov es, ax     ; Устанавливаем ES на сегмент PSP исходныц
    mov ax, 4C00h            ; Функция DOS 4Ch - завершение программы
    int 21h                 ; Вызов прерывания DOS
